...
import { blue, yellow, red, bold } from 'chalk';
...
function logEntry(params) {
  ...
  let logMessage = `${prefix}${bold(level)} (${source}): ${text}`;
...
function logConsole(params) {
  ...
  let logMessage = `${prefix}${bold(`console.${type}`)} called`;
  log(color(logMessage));
  recordLogMessage(logMessage);
